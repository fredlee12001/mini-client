...
Test_M2MResourceInstance::Test_M2MResourceInstance()
{
    ...
    handler = new Handler();
    obj = new M2MObject(3, "name");
    m2mresource_stub::object_instance = new M2MObjectInstance(*obj, 100, "name", "", "");
...
{
    // Catch a pointer to the private member of resource_instance just as
    // the m2mbase_stub does not have static sn_resource context anymore.
    M2MBase::lwm2m_parameters_s &sn_resource = resource_instance->_sn_resource;

    sn_resource.data_type = M2MBase::STRING;

    CHECK(resource_instance->resource_instance_type() == M2MResourceBase::STRING);

    sn_resource.data_type = M2MBase::INTEGER;

    CHECK(resource_instance->resource_instance_type() == M2MResourceBase::INTEGER);

    sn_resource.data_type = M2MBase::FLOAT;

    CHECK(resource_instance->resource_instance_type() == M2MResourceBase::FLOAT);

    sn_resource.data_type = M2MBase::BOOLEAN;

    CHECK(resource_instance->resource_instance_type() == M2MResourceBase::BOOLEAN);

    sn_resource.data_type = M2MBase::OPAQUE;

    CHECK(resource_instance->resource_instance_type() == M2MResourceBase::OPAQUE);

    sn_resource.data_type = M2MBase::TIME;

    CHECK(resource_instance->resource_instance_type() == M2MResourceBase::TIME);

    sn_resource.data_type = M2MBase::OBJLINK;

    CHECK(resource_instance->resource_instance_type() == M2MResourceBase::OBJLINK);
}
...
{
    CHECK(M2MBase::ResourceInstance == resource_instance->base_type());
}
...
void Test_M2MResourceInstance::test_set_value()
{
  ...
  //  resource_instance->_value = (u_int8_t*)malloc(sizeof(u_int8_t));
    ...
    resource_instance->_sn_resource.data_type = M2MBase::STRING;

    CHECK(resource_instance->set_value(value,(u_int32_t)sizeof(value)) == true);

    resource_instance->_sn_resource.data_type = M2MBase::INTEGER;
    ...
    free(resource_instance->_sn_resource.dynamic_resource_params.resource);
    resource_instance->_sn_resource.dynamic_resource_params.resourcelen = 0;
    ...
    resource_instance->_sn_resource.dynamic_resource_params.resource = (u_int8_t*)malloc(sizeof(value)+1);
    memset(resource_instance->_sn_resource.dynamic_resource_params.resource,0,sizeof(value)+1);
    memcpy(resource_instance->_sn_resource.dynamic_resource_params.resource,value,sizeof(value));
    resource_instance->_sn_resource.dynamic_resource_params.resourcelen = sizeof(value);
    ...
    free(resource_instance->_sn_resource.dynamic_resource_params.resource);
    resource_instance->_sn_resource.dynamic_resource_params.resource = NULL;
    resource_instance->_sn_resource.dynamic_resource_params.resourcelen = 0;
    ...
    uint8_t *set_value = (uint8_t*)malloc(10);
    memcpy(set_value, "a", 10);
    CHECK(resource_instance->set_value_raw(set_value, 10) == true);
    free(m2mbase_stub::nsdl_resource->resource);
    m2mbase_stub::nsdl_resource->resource = NULL;

    CHECK(resource_instance->set_value_raw(NULL, 10) == false);
...
void Test_M2MResourceInstance::test_get_value()
{
    ...
    resource_instance->set_value(test_value, value_length);

    resource_instance->get_value(buffer, val_size);
    CHECK(val_size == value_length);
    CHECK(memcmp(test_value, buffer, value_length) == 0);
...
void Test_M2MResourceInstance::test_handle_get_request()
{
    ...
    // OMA OPAQUE
    resource_instance->_sn_resource.data_type = M2MBase::OPAQUE;
    ...
    coap_header->options_list_ptr->accept = COAP_CT_NONE;
    CHECK(resource_instance->handle_get_request(NULL,coap_header,handler) != NULL);

    resource_instance->_sn_resource.data_type = M2MBase::INTEGER;
    CHECK(resource_instance->handle_get_request(NULL,coap_header,handler) != NULL);

    coap_header->options_list_ptr->accept = sn_coap_content_format_e(COAP_CONTENT_OMA_TLV_TYPE);
    CHECK(resource_instance->handle_get_request(NULL,coap_header,handler) != NULL);
...
{
    m2mbase_stub::int_value = 3;
    CHECK(resource_instance->object_id() == 3);
}
...
{
    uint8_t value[] = {"1234"};
    m2mbase_stub::string_value = "2/0/12/43";

    M2MResourceInstance* res_inst = new M2MResourceInstance(*m2mobjectinstance_stub::resource,
                                                            43,
                                                            M2MBase::Dynamic,
                                                            "res_type",
                                                            M2MBase::STRING,
                                                            ...
                                                            false);
    CHECK(res_inst != NULL);
    STRCMP_EQUAL("2/0/12/43", res_inst->uri_path());
...
void Test_M2MResourceInstance::test_get_parent_resource()
{
    ...
    CHECK(m2mobjectinstance_stub::resource != NULL);

    // Only for the code coverage
    CHECK(m2mobjectinstance_stub::resource == &res_inst->get_parent_resource());

    delete res_inst;
}

void Test_M2MResourceInstance::test_set_observation_handler()
{
    resource_instance->set_observation_handler(handler);
    CHECK(resource_instance->observation_handler() == handler);
}
