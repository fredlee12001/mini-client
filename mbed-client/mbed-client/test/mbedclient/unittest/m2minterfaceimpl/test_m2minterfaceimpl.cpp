...
{
public:
    M2MBaseTest() : M2MBase(M2MBase::Resource,
                            7,
     M2MBase::Static,
...
void Test_M2MInterfaceImpl::test_register_object()
{
    ...
    M2MObject *object = new M2MObject(1, "test");
    M2MObjectInstance *ins = object->create_object_instance();
    ins->create_dynamic_resource(2,"type",M2MResourceInstance::STRING,false,false);
...
void Test_M2MInterfaceImpl::test_registration_error()
{
    ...
    observer->error_occured = false;
    impl->registration_error(M2MInterface::UnregistrationFailed, true);
    CHECK(observer->error_occured == true);

    observer->error_occured = false;
    impl->registration_error(M2MInterface::UnregistrationFailed, false);
    CHECK(observer->error_occured == true);
    STRCMP_EQUAL(impl->_error_description, ERROR_REASON_24);
}
...
void Test_M2MInterfaceImpl::test_bootstrap_error_wait()
{
    impl->bootstrap_error_wait("bs-error");

    CHECK(impl->_current_state == M2MInterfaceImpl::STATE_BOOTSTRAP_ERROR_WAIT);
    STRCMP_EQUAL(impl->_error_description, "bs-error");
    CHECK(observer->bootstrapped == false);
}
...
void Test_M2MInterfaceImpl::test_bootstrap_error()
{
    ...
    for (int i = 0; i <= MBED_CLIENT_RECONNECTION_COUNT; i++) {
        impl->bootstrap_error("server-error");
...
void Test_M2MInterfaceImpl::test_socket_error()
{
    ...
    observer->error_occured = false;
    impl->_queue_mode_timer_ongoing = true;
    ...
    impl->socket_error(M2MConnectionHandler::SOCKET_SEND_ERROR, true);
    CHECK(observer->error_occured == false);
}
...
void Test_M2MInterfaceImpl::test_address_ready()
{
    ...
    impl->_reconnection_state = M2MInterfaceImpl::Unregistration;
    impl->address_ready(*address,server_type,server_port);
    CHECK(impl->_current_state == M2MInterfaceImpl::STATE_WAITING);
...
void Test_M2MInterfaceImpl::test_data_sent()
{
    ...
    impl->_current_state = M2MInterfaceImpl::STATE_BOOTSTRAP_WAIT;
    impl->data_sent();
    CHECK(impl->_current_state == M2MInterfaceImpl::STATE_BOOTSTRAPPED);

    observer->error_occured = false;
    impl->_current_state = M2MInterfaceImpl::STATE_BOOTSTRAP_ERROR_WAIT;
    impl->data_sent();
    CHECK(impl->_current_state == M2MInterfaceImpl::STATE_IDLE);
    CHECK(observer->error_occured == true);
}
...
void Test_M2MInterfaceImpl::test_timer_expired()
{
    ...
    /* Test reconnecting part in state_bootstrap() */
    impl->_security = new M2MSecurity(M2MSecurity::Bootstrap);
    impl->_bootstrapped = false;
    impl->timer_expired(M2MTimerObserver::RetryTimer);
    CHECK(impl->_retry_timer_expired == true);
    delete impl->_security;
