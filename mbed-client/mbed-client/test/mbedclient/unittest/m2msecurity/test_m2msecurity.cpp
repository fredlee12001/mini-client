...
{
    m2mbase_stub::free_path_member_on_delete = true;
    M2MObject* obj = new M2MObject(0, M2MBase::stringdup("0"));
    m2mobject_stub::inst = new M2MObjectInstance(*obj, 0, "name", M2MBase::stringdup("0"), "name");
...
void Test_M2MSecurity::test_delete_resource()
{
    ...
    CHECK(sec->delete_resource(M2MSecurity::ShortServerID) == false);
    delete sec;
...
void Test_M2MSecurity::test_resource_value_int()
{
    ...
    // the value needs to be copied to heap as the clear_resources() will free() the old value
    m2mresourcebase_stub::value = (uint8_t*)malloc(sizeof(value));
    memcpy(m2mresourcebase_stub::value, value, size);
    m2mresourcebase_stub::int_value = size;
...
void Test_M2MSecurity::test_set_bootstrap_flow()
{
    security->set_bootstrap_flow(true);
    CHECK(true == security->_is_bootstrap_flow);
}

void Test_M2MSecurity::test_is_bootstrap_flow()
{
    security->_is_bootstrap_flow = true;
    CHECK(true == security->is_bootstrap_flow());
}
