...
void Test_M2MTLVSerializer::test_serialize_object()
{
    ...
    // test with max object id, should be ok
    m2mbase_stub::operation = M2MBase::GET_ALLOWED;
    m2mbase_stub::name_id_value = 0xffff;
    data = serializer->serialize( m2mobject_stub::instance_list,size);
    CHECK(data != NULL);
    free(data);
...
void Test_M2MTLVSerializer::test_serialize_object_instance()
{
    ...
    // Only GET resources are part of TLV payload
    m2mbase_stub::operation = M2MBase::DELETE_ALLOWED;
    data = serializer->serialize_list(instance->resources(), size);
    CHECK(data == NULL);
...
void Test_M2MTLVSerializer::test_serialize_resource(M2MResourceBase::ResourceType resourceType, uint8_t *test_value)
{
    M2MObject *object = new M2MObject(1,"1");
    M2MObjectInstance* instance =new M2MObjectInstance(*object, 0, "1","type", "1");
    ...
    m2mbase_stub::operation = M2MBase::GET_ALLOWED;
    m2mresourcebase_stub::resource_type = resourceType;
    m2mresourcebase_stub::value = test_value;
...
{
    uint8_t value[] = {"1234"};
    uint8_t value_bool[] = {"1"};

    // resouce type INTEGER
    test_serialize_resource(M2MResourceBase::INTEGER, value);

    // resouce type STRING
    test_serialize_resource(M2MResourceBase::STRING, value);

    // resouce type BOOLEAN
    test_serialize_resource(M2MResourceBase::BOOLEAN, value_bool);
}

void Test_M2MTLVSerializer::test_serialize_resource_instance(M2MResourceBase::ResourceType resourceType, uint8_t *test_value)
{
    M2MObject *object = new M2MObject(1,"1");
    M2MObjectInstance* instance = new M2MObjectInstance(*object, 0, "1","type", "1");
...
void Test_M2MTLVSerializer::test_serialize_resource_instance()
{
    ...
    // resouce type INTEGER
    test_serialize_resource_instance(M2MResourceBase::INTEGER, value);

    // resouce type STRING
    test_serialize_resource_instance(M2MResourceBase::STRING, value);

    // resouce type BOOLEAN
    test_serialize_resource_instance(M2MResourceBase::BOOLEAN, value_bool);
}
