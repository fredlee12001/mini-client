...
public:
    ...
    void resource_to_be_deleted(M2MBase *){_visited=true;}
    void remove_object(M2MBase *){_visited = true;}
    void value_updated(M2MBase *){_visited = true;}

    void clear() {_visited = false;}
    bool _visited;
};
...
{
    M2MObject *object = new M2MObject(3, "3");

    STRCMP_EQUAL(object->uri_path(), "3");
...
void Test_M2MObject::test_create_object_instance()
{
    ...
    // The m2mbase_stub's create_path() uses a shortcut to path, which sould be fixed
    // (which breaks a few other places). Until then, hardwire the path to stub.
    m2mbase_stub::path = "1/0";

    M2MObjectInstance *obj_inst = object->create_object_instance(id);
    CHECK(obj_inst != NULL);
    CHECK(obj_inst->name_id() == 0);

    m2mbase_stub::path = "1/12";

    obj_inst = object->create_object_instance(12);
    CHECK(obj_inst != NULL);
    CHECK(obj_inst->name_id() == 12);
}

void Test_M2MObject::test_remove_object_instance()
{
    // the stub uses this value as object instance name id
    m2mbase_stub::name_id_value = 0;

    M2MObjectInstance *ins = new M2MObjectInstance(*object, 0, "name","type", "");
...
void Test_M2MObject::test_object_instance()
{
    ...
    M2MObjectInstance *obj_inst = object->object_instance(0);
...
void Test_M2MObject::test_instances()
{
    ...
    M2MObjectInstance *ins1 = new M2MObjectInstance(*object, 1, "name","type", "");
    ...
    M2MObjectInstance *obj_inst = list[0];
    CHECK(2 == list.size());
    CHECK(obj_inst->name_id() == 0);
}
...
void Test_M2MObject::test_handle_get_request()
{
    ...
    m2mtlvserializer_stub::uint8_value = (uint8_t*)malloc(1);
    // OMA JSON
    coap_header->options_list_ptr->accept = sn_coap_content_format_e(COAP_CONTENT_OMA_JSON_TYPE);
    m2mbase_stub::uint16_value = 100;
    CHECK(object->handle_get_request(NULL,coap_header,handler) != NULL);
...
void Test_M2MObject::test_set_observation_handler()
{
    object->set_observation_handler(handler);
    CHECK(object->observation_handler() == handler);
}
