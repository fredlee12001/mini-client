...
{
    m2mbase_stub::free_path_member_on_delete = false;
}
...
{
    // The create_object is doing the path allocation at real implementation, not in stub as in other
    // tests, so the freeing needs to be enabled.
    m2mbase_stub::free_path_member_on_delete =  true;

    M2MObject *test = M2MInterfaceFactory::create_object(1);
    CHECK(test != NULL);
    delete test;
    test = NULL;
}
