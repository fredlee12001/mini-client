                                                    ...
                                                    const char *context_address)
{
    ...
    tr_debug("M2MInterfaceFactory::create_interface - parameters endpoint name : %s",endpoint_name);
    tr_debug("M2MInterfaceFactory::create_interface - parameters endpoint type : %s",endpoint_type);
    ...
    if(endpoint_type) {
        if(strlen(endpoint_type) > MAX_ALLOWED_STRING_LENGTH){
    ...
    if(domain) {
        if(strlen(domain) > MAX_ALLOWED_STRING_LENGTH){
            domain_valid = false;
        }
    }

    if(((life_time == -1) || (life_time >= MINIMUM_REGISTRATION_TIME)) &&
       endpoint_name && (strlen(endpoint_name) <= MAX_ALLOWED_STRING_LENGTH) &&
       endpoint_type_valid && domain_valid) {
        tr_debug("M2MInterfaceFactory::create_interface - Creating M2MInterfaceImpl");
        interface = new M2MInterfaceImpl(observer, listen_port,
                                         mode, stack);
        if(interface) {
            if(!interface->setup(endpoint_name, endpoint_type,
                                  life_time, domain, context_address)){
                delete interface;
                return NULL;
            }
        }
...
M2MSecurity* M2MInterfaceFactory::create_security(M2MSecurity::ServerType server_type)
{
    ...
    if(security) {
        if(!security->allocate_resources()) {
            delete security;
            security = NULL;
        }
    }
...
M2MObject* M2MInterfaceFactory::create_object(uint16_t name_id)
{
    tr_debug("M2MInterfaceFactory::create_object : Name id : %d", name_id);
    M2MObject *object = NULL;
    object = new M2MObject(name_id, NULL);
    return object;
}
