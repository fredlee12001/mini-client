...
M2MObject::M2MObject(uint16_t name_id, char *path, bool external_blockwise_store)
: M2MBase(M2MBase::Object,
          name_id,
          M2MBase::Dynamic,
          NULL,
          path,
...
M2MObject::~M2MObject()
{
    M2MObjectInstance* inst = _instance_list;
    while (inst) {
        M2MObjectInstance* next = inst->get_next();
...
M2MObjectInstance* M2MObject::create_object_instance(uint16_t instance_id)
{
    ...
    if(!object_instance(instance_id)) {
        ...
        instance = new M2MObjectInstance(*this, instance_id, "", NULL);
        if(instance) {
            instance->add_observation_level(observation_level());
            add_object_instance(instance);
        }
    }
    return instance;
}
...
bool M2MObject::remove_object_instance(uint16_t inst_id)
{
    ...
    bool success = false;
    M2MObjectInstance *inst = _instance_list;
    M2MObjectInstance *prev = NULL;

    while (inst) {
        M2MObjectInstance *next = inst->get_next();

        if (inst->name_id() == inst_id) {
...
M2MObjectInstance* M2MObject::object_instance(uint16_t inst_id) const
{
    ...
    M2MObjectInstance *found_inst = NULL;
    M2MObjectInstance *inst = _instance_list;
...
const M2MObjectInstance* M2MObject::instances() const
{
    return _instance_list;
}

M2MObjectInstance* M2MObject::instances()
{
    return _instance_list;
}
...
{
    uint16_t count = 0;
    const M2MObjectInstance *inst = _instance_list;
        ...
        if ((operation() & SN_GRS_GET_ALLOWED) != 0) {
            if(coap_response) {
                ...
                if (is_content_type_supported) {
                    ...
                    tr_debug("M2MObject::handle_get_request() - Request Content-Type %d", coap_response->content_format);
...
{
    tr_debug("M2MObject::handle_put_request() %d", received_coap_header->content_format);
                            ...
                            switch(error) {
                                case M2MTLVDeserializer::None:
                                    ...
                                    if (coap_response->options_list_ptr) {
                                        ...
                                        // TODO: use helper
                                        if (obj_name.ensure_space(5 + 1 + 5 + 1)) {
                                            obj_name.append_int(M2MBase::name_id());
                                            obj_name.append('/');
...
void M2MObject::add_object_instance(M2MObjectInstance *object_instance)
{
    ...
    if (object_instance) {
        ...
        assert(object_instance->get_next() == NULL);
        ...
        M2MObjectInstance* last = _instance_list;

        if (last == NULL) {
            _instance_list = object_instance;
        } else {
            // loop until we find the last one
            while ((last != NULL) && (last->get_next() != NULL)) {
                last = last->get_next();
                assert(last != object_instance); // verify that we are not adding the same object there twice
            }
            last->set_next(object_instance);
            object_instance->set_next(NULL);
        }
    }
}

void M2MObject::uri_path(path_buffer &buffer) const
{
