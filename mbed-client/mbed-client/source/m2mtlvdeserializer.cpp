                                                                           ...
                                                                           bool update_value)
{
    ...
    M2MObjectInstance *instance = object.instances();

    if (TYPE_OBJECT_INSTANCE == til._type) {
        for (; instance != NULL; instance = instance->get_next()) {
            if(instance->instance_id() == til._id) {
                error = deserialize_resources(tlv, tlv_size, offset, *instance, operation, update_value);
            }
        }
        offset += til._length;
    ...
    if (TYPE_RESOURCE == til._type || TYPE_RESOURCE_INSTANCE == til._type) {
            ...
            if (resource->name_id() == til._id){
                ...
                if(update_value) {
                    if(til._length > 0) {
                        ...
                        resource->clear_value();
                    }
                    break;
                } else if(0 == (resource->operation() & SN_GRS_PUT_ALLOWED)) {
        ...
        if(!found) {
            if(M2MTLVDeserializer::Post == operation) {
                ...
                M2MResource *resource = object_instance.create_dynamic_resource(til._id, "",M2MResourceInstance::OPAQUE,true,false);
                if(resource) {
                                                                             ...
                                                                             bool update_value)
{
    ...
    if (TYPE_MULTIPLE_RESOURCE == til._type || TYPE_RESOURCE_INSTANCE == til._type) {
        M2MResourceInstance *instance = resource.resource_instances();
        bool found = false;
        for (; instance != NULL; instance = instance->get_next()) {
            if(instance->instance_id() == til._id && TYPE_RESOURCE_INSTANCE == til._type) {
                found = true;
                if(update_value) {
                    if(til._length > 0) {
                        if (!set_resource_instance_value(instance, tlv+offset, til._length)) {
                            error = M2MTLVDeserializer::OutOfMemory;
                            break;
                        }
                    } else {
                        instance->clear_value();
                    }
                    break;
                } else if(0 == (instance->operation() & SN_GRS_PUT_ALLOWED)) {
                    error = M2MTLVDeserializer::NotAllowed;
                    break;
                }
            }
        ...
        if(!found) {
            if(M2MTLVDeserializer::Post == operation) {
                ...
                M2MResourceInstance *res_instance = object_instance.create_dynamic_resource_instance(resource.name_id(), "",
                                                                             ...
                                                                             bool update_value)
{
    ...
    if (TYPE_RESOURCE_INSTANCE == til._type) {
        // XXX: this whole method is mostly a copypaste of deserialize_resource_instances
        M2MResourceInstance *instance = resource.resource_instances();
        bool found = false;
        for (; instance != NULL; instance = instance->get_next()) {
            if (instance->instance_id() == til._id) {
                                          ...
                                          uint32_t offset_size)
{
    ...
    for (; resource != NULL; resource = resource->get_next()) {
        ...
        if (!found) {
            tr_debug("M2MTLVDeserializer::remove_resources - remove resource %d", resource->name_id());
            object_instance.remove_resource(resource->name_id());
        } else {
            resource = resource->get_next();
        }
    }
}
                                          ...
                                          uint32_t offset_size)
{
    ...
    for (; instance != NULL; instance = instance->get_next()) {
        ...
        if (!found) {
            tr_debug("M2MTLVDeserializer::remove_resource_instances - remove resource instance %d", instance->instance_id());
            resource.remove_resource_instance(instance->instance_id());
        } else {
