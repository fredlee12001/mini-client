...
bool M2MFirmware::allocate_resources()
{
    if(NULL == (_firmware_instance = M2MObject::create_object_instance())) {
        return false;
    }
...
: M2MObject(M2M_FIRMWARE_ID, NULL)
{
}
...
bool M2MFirmware::create_mandatory_resources()
{
    ...
    // of parameters.
    res = _firmware_instance->create_dynamic_resource(FIRMWARE_PACKAGE,
                                                      OMA_RESOURCE_TYPE,
    ...
    // There should be canonical default values.
    res->set_value(0);
...
M2MResource* M2MFirmware::create_resource(FirmwareResource resource, const String &value)
{
    ...
    if ((firmware_id >= 0) && (value.size() < 256)) {
...
uint16_t M2MFirmware::resource_name(FirmwareResource resource)
{
    int res_name = -1;
    switch(resource) {
        case Package:
...
uint16_t M2MFirmware::total_resource_count() const
{
    uint16_t count = 0;
    if(_firmware_instance) {
        count = _firmware_instance->total_resource_count();
    }
    return count;
}
...
M2MResource* M2MFirmware::get_resource(FirmwareResource res) const
{
    ...
    if(_firmware_instance) {
        // TODO: remove this (and other similar) mapper completely
        uint16_t res_name;
        switch(res) {
            case Package:
                res_name = FIRMWARE_PACKAGE;
