...
char* String::strdup(const char* s)
{
    ...
    char *p2 = static_cast<char*>(memory_alloc(len));
    if (p2) {
        memcpy(p2, s, len);
        allocated_ = len;
        size_ = len-1;
...
String::String(const String& s)
    : p(0)
{
    p = static_cast<char*>(memory_alloc(s.size_ + 1));
...
{
    p = static_cast<char*>(memory_alloc(n + 1));
...
void String::new_realloc( size_type n) {
    if (n > 0 ) {
        ...
        //trying to have rough implementation of realloc
        char* pnew = static_cast<char*>(memory_alloc(n)); // could return NULL
        if (pnew) {
            memcpy(pnew, p, n);
            memory_free(p);
            p = pnew;
...
uint8_t* String::convert_integer_to_array(int64_t value, uint8_t &size, const uint8_t *array, const uint32_t array_size)
{
    ...
    buffer = (uint8_t*)memory_alloc(size);
    if (buffer) {
