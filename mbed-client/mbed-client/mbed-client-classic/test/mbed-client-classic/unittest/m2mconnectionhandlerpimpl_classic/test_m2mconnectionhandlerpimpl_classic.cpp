...
void Test_M2MConnectionHandlerPimpl_classic::test_dns_handler()
{
    ...
    udp4_unsec->_socket_state = M2MConnectionHandlerPimpl::EsocketStateInitilalizeConnection;
    ...
    udp4_unsec->resolve_server_address("1", 12345, M2MConnectionObserver::LWM2MServer, NULL);
    udp4_unsec->_socket_state = M2MConnectionHandlerPimpl::EsocketStateInitilalizeConnection;
    udp4_unsec->_socket_address.addressType = PAL_AF_INET;
    udp4_unsec->dns_handler();
    ...
    udp4_unsec->resolve_server_address("1", 12345, M2MConnectionObserver::LWM2MServer, NULL);
    udp4_unsec->_socket_state = M2MConnectionHandlerPimpl::EsocketStateInitilalizeConnection;
    udp4_unsec->_socket_address.addressType = PAL_AF_INET6;
    udp4_unsec->dns_handler();
    CHECK(observer->addressReady == true);
    CHECK(observer->error == false);
    ...
    tcp4_unsec->_socket_state = M2MConnectionHandlerPimpl::EsocketStateInitilalizeConnection;
    tcp4_unsec->_socket_address.addressType = PAL_AF_INET;
    ...
    udp4_sec->_socket_state = M2MConnectionHandlerPimpl::EsocketStateInitilalizeConnection;
    udp4_sec->_socket_address.addressType = PAL_AF_INET;
    ...
    pal_network_stub::socket_address.addressType = PAL_AF_INET6;
    udp4_unsec->resolve_server_address("1", 12345, M2MConnectionObserver::LWM2MServer, NULL);
    udp4_unsec->_socket_state = M2MConnectionHandlerPimpl::EsocketStateInitilalizeConnection;
    udp4_unsec->dns_handler();
    CHECK(observer->addressReady == false);
    ...
    tcp6_unsec->_socket_state = M2MConnectionHandlerPimpl::EsocketStateInitilalizeConnection;
    tcp6_unsec->dns_handler();
    CHECK(observer->addressReady == false);
    CHECK(observer->error);

    observer->error = false;
    tcp6_unsec->_socket_address.addressType = PAL_AF_INET6;
    pal_network_stub::status = PAL_SUCCESS;
    pal_network_stub::new_status = PAL_ERR_GENERIC_FAILURE;
    pal_network_stub::change_status_count = 1;
    tcp6_unsec->resolve_server_address("1", 12345, M2MConnectionObserver::LWM2MServer, NULL);
    ...
    pal_network_stub::status = PAL_SUCCESS;
    pal_network_stub::new_status = PAL_ERR_GENERIC_FAILURE;
    pal_network_stub::change_status_count = 5;
    tcp6_unsec->resolve_server_address("1", 12345, M2MConnectionObserver::LWM2MServer, NULL);
    tcp6_unsec->_socket_state = M2MConnectionHandlerPimpl::EsocketStateInitilalizeConnection;
    tcp6_unsec->dns_handler();
    CHECK(observer->addressReady == false);
    ...
    pal_network_stub::status = PAL_SUCCESS;
    pal_network_stub::new_status = PAL_ERR_GENERIC_FAILURE;
    pal_network_stub::change_status_count = 6;
    tcp6_unsec->resolve_server_address("1", 12345, M2MConnectionObserver::LWM2MServer, NULL);
    tcp6_unsec->_socket_state = M2MConnectionHandlerPimpl::EsocketStateInitilalizeConnection;
    tcp6_unsec->dns_handler();
    CHECK(observer->addressReady == false);
    ...
    pal_network_stub::status = PAL_SUCCESS;
    pal_network_stub::new_status = PAL_ERR_GENERIC_FAILURE;
    pal_network_stub::change_status_count = 7;
    tcp6_unsec->resolve_server_address("1", 12345, M2MConnectionObserver::LWM2MServer, NULL);
    tcp6_unsec->_socket_state = M2MConnectionHandlerPimpl::EsocketStateInitilalizeConnection;
    tcp6_unsec->dns_handler();
    CHECK(observer->addressReady == false);
    ...
    observer->error = false;
    pal_network_stub::status = PAL_SUCCESS;
    pal_network_stub::new_status = PAL_ERR_SOCKET_IN_PROGRES;
    pal_network_stub::change_status_count = 8;
    tcp6_unsec->resolve_server_address("1", 12345, M2MConnectionObserver::LWM2MServer, NULL);
    tcp6_unsec->_socket_state = M2MConnectionHandlerPimpl::EsocketStateInitilalizeConnection;
    tcp6_unsec->dns_handler();
    ...
    pal_network_stub::status = PAL_SUCCESS;
    handler->_socket_state = M2MConnectionHandlerPimpl::ESocketStateConnecting;
    handler->dns_handler();
    ...
    pal_network_stub::status = PAL_SUCCESS;
    handler->_security = (const M2MSecurity*)1;
    handler->_socket_state = M2MConnectionHandlerPimpl::ESocketStateConnected;
    handler->dns_handler();
    handler->_security = (const M2MSecurity*)0;

    pal_network_stub::status = PAL_SUCCESS;
    m2mconnectionsecurityimpl_stub::int_value = 1;
    udp4_sec->_socket_state = M2MConnectionHandlerPimpl::ESocketStateConnected;
    udp4_sec->dns_handler();
...
void Test_M2MConnectionHandlerPimpl_classic::test_send_data()
{
    ...
    handler->_socket_state = M2MConnectionHandlerPimpl::ESocketStateDisconnected;
    CHECK(false == handler->send_data(data, 5 , addr));

    handler->_socket_state = M2MConnectionHandlerPimpl::ESocketStateUnsecureConnection;
...
void Test_M2MConnectionHandlerPimpl_classic::test_send_socket_data()
{
    ...
    udp4_unsec->_socket_state = M2MConnectionHandlerPimpl::ESocketStateUnsecureConnection;
    tcp4_unsec->_socket_state = M2MConnectionHandlerPimpl::ESocketStateUnsecureConnection;
    udp4_sec->_socket_state = M2MConnectionHandlerPimpl::ESocketStateSecureConnection;
    tcp4_sec->_socket_state = M2MConnectionHandlerPimpl::ESocketStateSecureConnection;
    ...
    udp4_unsec->_socket_state = M2MConnectionHandlerPimpl::ESocketStateDisconnected;
    observer->dataSent = false;
    udp4_unsec->send_socket_data(data, sizeof(data));
    CHECK(!observer->dataSent);
...
void Test_M2MConnectionHandlerPimpl_classic::test_receive_handler()
{
    ...
    udp4_sec->_socket_state = M2MConnectionHandlerPimpl::ESocketStateHandshaking;
    observer->error = 0;
    observer->dataAvailable = false;
    m2mconnectionsecurityimpl_stub::use_inc_int = false;
    m2mconnectionsecurityimpl_stub::int_value = M2MConnectionHandler::CONNECTION_ERROR_WANTS_READ;
    ...
    udp4_sec->_socket_state = M2MConnectionHandlerPimpl::ESocketStateDisconnected;
    observer->error = 0;
    observer->dataAvailable = false;
    m2mconnectionsecurityimpl_stub::use_inc_int = false;
    m2mconnectionsecurityimpl_stub::int_value = M2MConnectionHandler::CONNECTION_ERROR_WANTS_READ;
...
void Test_M2MConnectionHandlerPimpl_classic::test_send_to_socket()
{
    ...
    udp4_unsec->_socket_state = M2MConnectionHandlerPimpl::ESocketStateHandshaking;
    tcp4_unsec->_socket_state = M2MConnectionHandlerPimpl::ESocketStateHandshaking;
...
void Test_M2MConnectionHandlerPimpl_classic::test_connection_event_handler()
{
    ...
    event.event_type = M2MConnectionHandlerPimpl::ESocketDnsResolved;
    connection_event_handler(&event);

    event.event_type = M2MConnectionHandlerPimpl::ESocketDnsError;
    connection_event_handler(&event);
...
void Test_M2MConnectionHandlerPimpl_classic::test_address_resolver()
{
    eventOS_stub::int8_value = 0;
    pal_network_stub::status = PAL_SUCCESS;
    handler->_server_address = "";
    handler->address_resolver();
...
void Test_M2MConnectionHandlerPimpl_classic::test_handle_dns_result()
{

    observer->error = 0;
    handler->_socket_state = M2MConnectionHandlerPimpl::ESocketStateHandshaking;
    handler->handle_dns_result(false);
    CHECK(!observer->error);

    observer->error = 0;
    handler->_socket_state = M2MConnectionHandlerPimpl::ESocketStateDNSResolving;
    handler->handle_dns_result(false);
    CHECK(observer->error);

    handler->_socket_address.addressType = PAL_AF_INET;
    pal_network_stub::status = PAL_SUCCESS;
    observer->error = 0;
    handler->_socket_state = M2MConnectionHandlerPimpl::ESocketStateDNSResolving;
    handler->handle_dns_result(true);
    CHECK(!observer->error);

}

void Test_M2MConnectionHandlerPimpl_classic::test_send_receive_event()
{

    eventOS_stub::int8_value = 0;
    handler->_socket_state = M2MConnectionHandlerPimpl::EsocketStateInitilalizeConnection;
    handler->send_receive_event();
...
void Test_M2MConnectionHandlerPimpl_classic::test_close_socket()
{

    handler->_socket = (void*)1;
