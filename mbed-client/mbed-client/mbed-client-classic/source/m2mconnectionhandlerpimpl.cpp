    ...
    switch (event->event_type) {
        ...
        case M2MConnectionHandlerPimpl::ESocketSend:
            ...
            if(pimpl) {
                ...
                if (task_id->data_ptr) {
                    M2MCoreMemory::memory_free(task_id->data_ptr);
                }
            }
            break;
        default:
...
bool M2MConnectionHandlerPimpl::resolve_server_address(const char *server_address,
...
void M2MConnectionHandlerPimpl::dns_handler()
{
    ...
    _socket_address = new SocketAddress(_net_iface,(const char*)_server_address, _server_port);
    if(*_socket_address) {
                                          ...
                                          sn_nsdl_addr_s *address)
{
    ...
    uint8_t *buffer = (uint8_t*)M2MCoreMemory::memory_alloc(data_len);
    if(!buffer) {
        return false;
    }
    ...
    TaskIdentifier* task = memory_pool.alloc();
    if (!task) {
        M2MCoreMemory::memory_free(buffer);
        return false;
    }
    task->pimpl = this;
...
void M2MConnectionHandlerPimpl::receive_handler()
{
    ...
    if(_listening) {
        if( _use_secure_connection ){
            int rcv_size = _security_impl->read(recv_buffer, receive_length);
            ...
            int recv = -1;
            if(is_tcp_connection()){
                recv = ((TCPSocket*)_socket)->recv(recv_buffer, receive_length);

            }else{
                recv = ((UDPSocket*)_socket)->recvfrom(NULL, recv_buffer, receive_length);
            }
            if (recv > 0) {
                ...
                if(is_tcp_connection()){
                    ...
                    if( receive_length > 4 ){
                        uint64_t len = (recv_buffer[0] << 24 & 0xFF000000) + (recv_buffer[1] << 16 & 0xFF0000);
                        len += (recv_buffer[2] << 8 & 0xFF00) + (recv_buffer[3] & 0xFF);
                        if(len > 0) {
                            uint8_t* buf = (uint8_t*)malloc(len);
                            if(buf) {
                                memmove(buf, recv_buffer+4, len);
