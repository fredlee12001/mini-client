...
typedef void (*buffer_callback)(const uint8_t *&, uint32_t&);
...
private:
    ...
    /**
     * \brief Memory allocations for the security object.
...
public:
    ...
    /**
     * \brief Sets if this object is created for complete bootstrap and
     * LWM2M server flow or not.
     * \param is_bootstrap_flow, true if object handles bootstrap and LWM2M flow else false.
     */
    void set_bootstrap_flow(bool is_bootstrap_flow);

    /**
     * \brief Returns if this object is created for complete bootstrap and
     * LWM2M server flow or not.
     * \return true, if object handles bootstrap and LWM2M flow else false.
     */
    bool is_bootstrap_flow() const;

    void set_device_certificate(buffer_callback callback);

    void set_server_certificate(buffer_callback callback);

    void set_device_key(buffer_callback callback);

    buffer_callback get_device_certificate() const;

    buffer_callback get_server_certificate() const;

    buffer_callback get_device_key() const;
...
private:
    ...
    bool                  _is_bootstrap_flow;
    buffer_callback       _device_certificate;
    buffer_callback       _server_certificate;
    buffer_callback       _device_key;
