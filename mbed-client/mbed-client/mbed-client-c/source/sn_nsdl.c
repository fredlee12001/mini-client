...
static uint8_t      name_parameter[]            = {'n', 'a', 'm', 'e', '='};      /* Name attribute. Only once */
/* * OMA BS parameters * */
...
static int8_t           sn_nsdl_process_internal_coap(struct nsdl_s *nsdl_handle, sn_coap_hdr_s *coap_packet_ptr, sn_nsdl_addr_s *src_addr_ptr);
...
static uint8_t coap_tx_callback(uint8_t *data_ptr, uint16_t data_len, sn_nsdl_addr_s *address_ptr, void *param)
{
    ...
    return handle->sn_nsdl_tx_callback(handle, SN_NSDL_PROTOCOL_COAP, data_ptr, data_len, address_ptr);
}
...
struct nsdl_s *sn_nsdl_init(uint8_t (*sn_nsdl_tx_cb)(struct nsdl_s *, sn_nsdl_capab_e , uint8_t *, uint16_t, sn_nsdl_addr_s *),
                            ...
                            sn_nsdl_dynamic_resource_parameters_s* (*sn_nsdl_get_first_cb)(struct nsdl_s *, struct iterator_state_s **, char **),
                            sn_nsdl_dynamic_resource_parameters_s* (*sn_nsdl_get_next_cb)(struct nsdl_s *, struct iterator_state_s **, char **),
                            sn_nsdl_dynamic_resource_parameters_s* (*sn_nsdl_find_resource_cb)(struct nsdl_s *, const char *),
                            uint8_t (*sn_dyn_res_callback)(struct nsdl_s *,sn_coap_hdr_s *, sn_nsdl_addr_s *, sn_nsdl_capab_e))
{
    ...
    handle->sn_nsdl_get_first_callback = sn_nsdl_get_first_cb;
    handle->sn_nsdl_get_next_callback = sn_nsdl_get_next_cb;
    handle->sn_nsdl_find_resource_callback = sn_nsdl_find_resource_cb;
    handle->sn_dyn_res_callback = sn_dyn_res_callback;
    ...
    //handle->coap = sn_coap_protocol_init(sn_nsdl_alloc, sn_nsdl_free, sn_nsdl_tx_cb, &sn_nsdl_local_rx_function);
    handle->coap = sn_coap_protocol_init(sn_nsdl_alloc, sn_nsdl_free, coap_tx_callback, &sn_nsdl_local_rx_function);
                                   ...
                                   uint32_t query_param_count)
{
    ...
    register_message_ptr = sn_coap_parser_alloc_message(handle->coap);
    ...
    if (sn_coap_parser_alloc_options(handle->coap, register_message_ptr) == NULL) {
        sn_coap_parser_release_allocated_coap_msg_mem(handle->coap, register_message_ptr);
...
uint16_t sn_nsdl_unregister_endpoint(struct nsdl_s *handle)
{
    ...
    if (sn_nsdl_is_ep_registered(handle)) {
        ...
        unregister_message_ptr = sn_coap_parser_alloc_message(handle->coap);
        ...
        if(handle->ep_information_ptr->location_ptr) {
            ...
            if (!unregister_message_ptr->uri_path_ptr) {
                sn_coap_parser_release_allocated_coap_msg_mem(handle->coap, unregister_message_ptr);
                return 0;
            }
...
int8_t sn_nsdl_set_endpoint_location(struct nsdl_s *handle, uint8_t *location_ptr, uint8_t location_len)
{
    ...
    handle->sn_nsdl_free(handle->ep_information_ptr->location_ptr);
    handle->ep_information_ptr->location_ptr = handle->sn_nsdl_alloc(location_len + 1);
    memset(handle->ep_information_ptr->location_ptr, 0, location_len + 1);
   ...
   sn_coap_content_format_e content_format)
{
    ...
    /* Check parameters */
    if (handle == NULL || handle->coap == NULL) {
        return 0;
    }

    /* Allocate and initialize memory for header struct */
    notification_message_ptr = sn_coap_parser_alloc_message(handle->coap);
    if (notification_message_ptr == NULL) {
        return 0;
    }

    if (sn_coap_parser_alloc_options(handle->coap, notification_message_ptr) == NULL) {
    ...
    sn_coap_parser_release_allocated_coap_msg_mem(handle->coap, notification_message_ptr);
                               ...
                               uint32_t query_param_count)
{
    ...
    if (!sn_coap_parser_alloc_options(handle->coap, &bootstrap_coap_header)) {
        return 0;
    }
    ...
    if(handle->oma_bs_address_ptr) {
        handle->sn_nsdl_free(handle->oma_bs_address_ptr);
        handle->oma_bs_address_ptr = NULL;
    }
...
int8_t sn_nsdl_process_coap(struct nsdl_s *handle, uint8_t *packet_ptr, uint16_t packet_len, sn_nsdl_addr_s *src_ptr)
{
    ...
    /* Parse CoAP packet */
    coap_packet_ptr = sn_coap_protocol_parse(handle->coap, src_ptr, packet_len, packet_ptr, (void *)handle);
    ...
    if(coap_packet_ptr->coap_status == COAP_STATUS_PARSER_BLOCKWISE_MSG_RECEIVING) {
        ...
        if(resource && resource->external_memory_block) {
            sn_coap_protocol_block_remove(handle->coap,
                                          src_ptr,
    ...
    if (coap_packet_ptr->options_list_ptr) {
        if (coap_packet_ptr->options_list_ptr->proxy_uri_len) {
            coap_response_ptr = sn_coap_build_response(handle->coap, coap_packet_ptr, COAP_MSG_CODE_RESPONSE_PROXYING_NOT_SUPPORTED);
            if (coap_response_ptr) {
                sn_nsdl_send_coap_message(handle, src_ptr, coap_response_ptr);
                sn_coap_parser_release_allocated_coap_msg_mem(handle->coap, coap_response_ptr);
                sn_coap_parser_release_allocated_coap_msg_mem(handle->coap, coap_packet_ptr);
                return SN_NSDL_SUCCESS;
            } else {
                sn_coap_parser_release_allocated_coap_msg_mem(handle->coap, coap_packet_ptr);
    ...
    /* * * * * * * * * * * * * * * */
    return sn_nsdl_process_internal_coap(handle, coap_packet_ptr, src_ptr);
}

int8_t sn_nsdl_exec(struct nsdl_s *handle, uint32_t time)
{
    if(!handle || !handle->coap){
        return SN_NSDL_FAILURE;
    }
    /* Call CoAP execution function */
    return sn_coap_protocol_exec(handle->coap, time);
}
...
sn_nsdl_dynamic_resource_parameters_s *sn_nsdl_get_resource(struct nsdl_s *handle, const char *path_ptr)
{
    ...
    return handle->sn_nsdl_find_resource_callback(handle, path_ptr);
}
...
static uint16_t sn_nsdl_internal_coap_send(struct nsdl_s *handle, sn_coap_hdr_s *coap_header_ptr, sn_nsdl_addr_s *dst_addr_ptr, uint8_t message_description)
{
    ...
    int8_t ret_val = prepare_blockwise_message(handle->coap, coap_header_ptr);
    if( 0 != ret_val ) {
        return 0;
    }
#endif

    coap_message_len = sn_coap_builder_calc_needed_packet_data_size_2(coap_header_ptr, handle->coap->sn_coap_block_data_size);
    ...
    /* Build message */
    if (sn_coap_protocol_build(handle->coap, dst_addr_ptr, coap_message_ptr, coap_header_ptr, (void *)handle) < 0) {
...
int8_t sn_nsdl_build_registration_body(struct nsdl_s *handle, sn_coap_hdr_s *message_ptr, uint8_t updating_registeration)
{
    ...
    char *uri_path = NULL;
    resource_temp_ptr = handle->sn_nsdl_get_first_callback(handle, &state, &uri_path);
    ...
    while (resource_temp_ptr) {
        ...
        if (resource_temp_ptr->publish_uri) {
            if (updating_registeration && resource_temp_ptr->registered == SN_NDSL_RESOURCE_REGISTERED) {
                resource_temp_ptr = handle->sn_nsdl_get_next_callback(handle, &state, &uri_path);
                continue;
            } else {
...
#ifdef RESOURCE_ATTRIBUTES_LIST
            /* Resource attributes */
            if (resource_temp_ptr->attributes_ptr) {
                sn_nsdl_attribute_item_s *attribute = resource_temp_ptr->attributes_ptr;
...
static uint16_t sn_nsdl_calculate_registration_body_size(struct nsdl_s *handle, uint8_t updating_registeration, int8_t *error)
{
    ...
    *error = SN_NSDL_SUCCESS;
    const sn_nsdl_dynamic_resource_parameters_s *resource_temp_ptr = NULL;
    ...
    while (resource_temp_ptr) {
        if (resource_temp_ptr->publish_uri) {
            ...
            if (resource_temp_ptr->attributes_ptr) {
                ...
                uint8_t success = 1;
                sn_nsdl_attribute_item_s *item = resource_temp_ptr->attributes_ptr;
...
static int8_t sn_nsdl_local_rx_function(struct nsdl_s *handle, sn_coap_hdr_s *coap_packet_ptr, sn_nsdl_addr_s *address_ptr)
{
    ...
    if (coap_packet_ptr->msg_code == COAP_MSG_CODE_RESPONSE_CREATED) {
        if (handle->coap->sn_coap_block_data_size > 0) {
            handle->register_msg_id += handle->register_msg_len / handle->coap->sn_coap_block_data_size;
        }
        if (coap_packet_ptr->msg_id == handle->register_msg_id) {
            handle->sn_nsdl_endpoint_registered = SN_NSDL_ENDPOINT_IS_REGISTERED;
            is_reg_msg = true;
            //sn_grs_mark_resources_as_registered(handle); //TODO:REFACTOR
        ...
        if (handle->coap->sn_coap_block_data_size > 0) {
            handle->update_register_msg_id += handle->update_register_msg_len / handle->coap->sn_coap_block_data_size;
        }
...
sn_grs_resource_list_s *sn_nsdl_list_resource(struct nsdl_s *handle, const char *path)
{
    ...
    //return sn_grs_list_resource(handle->grs, path); //TODO:REFACTOR
    return 0;
}
...
extern int8_t sn_nsdl_send_coap_message(struct nsdl_s *handle, sn_nsdl_addr_s *address_ptr, sn_coap_hdr_s *coap_hdr_ptr)
{
...
#if SN_COAP_MAX_BLOCKWISE_PAYLOAD_SIZE /* If Message blockwising is not used at all, this part of code will not be compiled */
    ret_val = prepare_blockwise_message(handle->coap, coap_hdr_ptr);
    if( 0 != ret_val ) {
        return SN_NSDL_FAILURE;
    }
#endif
    ...
    message_len = sn_coap_builder_calc_needed_packet_data_size_2(coap_hdr_ptr, handle->coap->sn_coap_block_data_size);
    ...
    message_ptr = handle->sn_nsdl_alloc(message_len);
    if (message_ptr == NULL) {
        return SN_NSDL_FAILURE;
    }

    /* Build CoAP message */
    if (sn_coap_protocol_build(handle->coap, address_ptr, message_ptr, coap_hdr_ptr, (void *)handle) < 0) {
        handle->sn_nsdl_free(message_ptr);
        message_ptr = 0;
        return SN_NSDL_FAILURE;
    }

    /* Call tx callback function to send message */
    ret_val = handle->sn_nsdl_tx_callback(handle, SN_NSDL_PROTOCOL_COAP, message_ptr, message_len, address_ptr);
...
extern sn_coap_hdr_s *sn_nsdl_build_response(struct nsdl_s *handle, sn_coap_hdr_s *coap_packet_ptr, uint8_t msg_code)
{
    ...
    return sn_coap_build_response(handle->coap, coap_packet_ptr, msg_code);
}
...
extern sn_coap_options_list_s *sn_nsdl_alloc_options_list(struct nsdl_s *handle, sn_coap_hdr_s *coap_msg_ptr)
{
    ...
    return sn_coap_parser_alloc_options(handle->coap, coap_msg_ptr);
}
...
extern void sn_nsdl_release_allocated_coap_msg_mem(struct nsdl_s *handle, sn_coap_hdr_s *freed_coap_msg_ptr)
{
    ...
    sn_coap_parser_release_allocated_coap_msg_mem(handle->coap, freed_coap_msg_ptr);
}
    ...
    uint8_t resending_count, uint8_t resending_interval)
{
    ...
    return sn_coap_protocol_set_retransmission_parameters(handle->coap,
        ...
        uint8_t buffer_size_messages, uint16_t buffer_size_bytes)
{
    ...
    return sn_coap_protocol_set_retransmission_buffer(handle->coap,
...
extern int8_t sn_nsdl_set_block_size(struct nsdl_s *handle, uint16_t block_size)
{
    ...
    return sn_coap_protocol_set_block_size(handle->coap, block_size);
}
...
extern int8_t sn_nsdl_set_duplicate_buffer_size(struct nsdl_s *handle, uint8_t message_count)
{
    ...
    return sn_coap_protocol_set_duplicate_buffer_size(handle->coap, message_count);
}
...
int8_t sn_nsdl_clear_coap_resending_queue(struct nsdl_s *handle)
{
    ...
    sn_coap_protocol_clear_retransmission_buffer(handle->coap);
    return SN_NSDL_SUCCESS;
}
#ifdef RESOURCE_ATTRIBUTES_LIST
...
void sn_nsdl_free_resource_attributes_list(sn_nsdl_dynamic_resource_parameters_s *params)
{
    if (params == NULL) {
        return;
    }
...
bool sn_nsdl_set_resource_attribute(sn_nsdl_dynamic_resource_parameters_s *params, const sn_nsdl_attribute_item_s *attribute)
{
    if (params == NULL) {
        return false;
    }
...
const char *sn_nsdl_get_resource_attribute(const sn_nsdl_dynamic_resource_parameters_s *params, sn_nsdl_resource_attribute_t attribute_name)
{
    char *value = NULL;
    if (params != NULL) {
        sn_nsdl_attribute_item_s *item = params->attributes_ptr;
...
bool sn_nsdl_remove_resource_attribute(sn_nsdl_dynamic_resource_parameters_s *params, sn_nsdl_resource_attribute_t attribute_name)
{
    if (params == NULL) {
        return false;
    }
...
int8_t sn_nsdl_process_internal_coap(struct nsdl_s *handle, sn_coap_hdr_s *coap_packet_ptr, sn_nsdl_addr_s *src_addr_ptr)
{
    tr_debug("sn_nsdl_process_internal_coap");
    if( !coap_packet_ptr || !handle){
        return SN_NSDL_FAILURE;
    }

    tr_debug("sn_nsdl_process_internal_coap - coap params:");
...
//        }
        ...
        resource_temp_ptr = handle->sn_nsdl_find_resource_callback(handle, path);
        handle->sn_nsdl_free(path);
        ...
        if (resource_temp_ptr) {
            ...
            if (resource_temp_ptr->mode == SN_GRS_DYNAMIC) {
                ...
                if (((coap_packet_ptr->msg_code == COAP_MSG_CODE_REQUEST_GET) && !(resource_temp_ptr->access & SN_GRS_GET_ALLOWED))          ||
                    ...
                    if (handle->sn_dyn_res_callback != NULL) {
                        handle->sn_dyn_res_callback(handle, coap_packet_ptr, src_addr_ptr, SN_NSDL_PROTOCOL_COAP);
                    }
        ...
        /* * * * * * * * * * * * * * */
            ...
            if (coap_packet_ptr->msg_code == COAP_MSG_CODE_REQUEST_POST) {
                sn_nsdl_local_rx_function(handle, coap_packet_ptr, src_addr_ptr);
    ...
    if (coap_packet_ptr->msg_type != COAP_MSG_TYPE_RESET && coap_packet_ptr->msg_type != COAP_MSG_TYPE_ACKNOWLEDGEMENT) {
        ...
        if (coap_packet_ptr->token_ptr) {
            ...
            response_message_hdr_ptr->token_ptr = handle->sn_nsdl_alloc(response_message_hdr_ptr->token_len);
        ...
        if (status == COAP_MSG_CODE_RESPONSE_CONTENT) {
            ...
            if (resource_temp_ptr->resourcelen != 0) {
                ...
                response_message_hdr_ptr->payload_ptr = handle->sn_nsdl_alloc(response_message_hdr_ptr->payload_len);
        ...
        sn_nsdl_send_coap_message(handle, src_addr_ptr, response_message_hdr_ptr);

        if (response_message_hdr_ptr->payload_ptr) {
            handle->sn_nsdl_free(response_message_hdr_ptr->payload_ptr);
