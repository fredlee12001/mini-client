...
#### The maximum time for CoAP message storing

This is a subconfiguration for the blockwise feature. The maximum time (in seconds) of storing the data (messages and payload) for the blockwise is configurable. After the timeout, the data packet is removed. Increasing the value gives more time for transferring larger amounts of data.
