...
int M2MConnectionSecurityPimpl::init(const M2MSecurity *security)
{
    ...
    if (security != NULL) {
        ...
        if( mbedtls_entropy_add_source( &_entropy, entropy_poll, NULL,
                                    128, 0 ) < 0 ){
            tr_debug("M2MConnectionSecurityPimpl::init - add entropy failed");
            return -1;
        }
        ...
        if( mbedtls_ctr_drbg_seed( &_ctr_drbg, mbedtls_entropy_func, &_entropy,
                                   ...
                                   strlen( pers ) )  != 0 ) {
            tr_debug("M2MConnectionSecurityPimpl::init - add ctr_drbg_seed failed");
            return -1;
        }
        ...
        if( mbedtls_ssl_config_defaults( &_conf,
                           ...
                           mode, 0 ) != 0 ) {
            tr_debug("M2MConnectionSecurityPimpl::init - add default ssl config failed");
            return -1;
        }
        ...
        if( srv_public_key == NULL || public_key == NULL || sec_key == NULL ||
            ...
            tr_debug("M2MConnectionSecurityPimpl::init - Retrieving PSK credentials failed");
            return -1;
        }
        ...
        if( cert_mode == M2MSecurity::Certificate ){
            ...
            tr_debug("M2MConnectionSecurityPimpl::init - add PSK credentials failed");
            ret = -1;
        }
...
void mbedtls_timing_set_delay( void *data, uint32_t int_ms, uint32_t fin_ms ){
    ...
    M2MDtlsTimer* timer = static_cast<M2MDtlsTimer*> (data);
    if(!timer) {
        return;
    }
    if( int_ms > 0 && fin_ms > 0 ){
        ...
        cancelled = false;
        timer->stop_timer();
        timer->start_timer(int_ms, fin_ms);
    }else{
