...
*     fcc_status_e.
*/
static fcc_status_e fcc_check_uri_contents(uint8_t* uri_data_buffer, size_t size_of_uri_data_buffer)
{
    ...
    uri_string = malloc(size_of_uri_data_buffer + 1);
    SA_PV_ERR_RECOVERABLE_RETURN_IF((uri_string == NULL), fcc_status = FCC_STATUS_MEMORY_OUT, "Failed to allocate memory for URI string");

    //Copy data and add '\0' to create uri string
    ...
    if (memcmp(uri_string, uri_coap_prefix, strlen(uri_coap_prefix)) != 0 && memcmp(uri_string, uri_coaps_prefix, strlen(uri_coaps_prefix)) != 0) {
        SA_PV_ERR_RECOVERABLE_GOTO_IF(true, fcc_status = FCC_STATUS_URI_WRONG_FORMAT, free, "Wrong uri prefix");
    }

    if ((strstr(uri_string, uri_aid_1) == NULL) && (strstr(uri_string, uri_aid_2) == NULL)) {
        SA_PV_ERR_RECOVERABLE_GOTO_IF(true, fcc_status = FCC_STATUS_URI_WRONG_FORMAT, free, "Wrong uri data");
    }
...
*        fcc_status_e status.
*/
static fcc_status_e fcc_get_certificate_attribute(const uint8_t *cert_data, size_t size_of_cert_data, cs_certificate_attribute_type_e attribute_type, uint8_t **attribute_data, size_t *attribute_size)
{
    ...
    //Get certificate CN field
    //Get size of "CN" attribute
    kcm_status = cs_attr_get_data_x509_cert(cert_data,
        size_of_cert_data,
        attribute_type,
        dummy_buf,
        0,
    ...
    SA_PV_ERR_RECOVERABLE_RETURN_IF((kcm_status != KCM_STATUS_INSUFFICIENT_BUFFER || *attribute_size == 0), fcc_status = FCC_STATUS_INVALID_CERT_ATTRIBUTE, "Failed to get size of attribute");

    *attribute_data = malloc(*attribute_size);
...
static fcc_status_e verify_existence_and_set_warning(const uint8_t *parameter_name, size_t size_of_parameter_name, kcm_item_type_e parameter_type,bool is_should_be_present)
{
    ...
    if (kcm_status != KCM_STATUS_SUCCESS &&  is_should_be_present == true) {
        output_info_fcc_status = fcc_store_warning_info((const uint8_t*)parameter_name, size_of_parameter_name, g_fcc_missing_parameter_warning_str);
    }
...
static fcc_status_e verify_server_uri(bool use_bootstrap)
{
    ...
    SA_PV_ERR_RECOVERABLE_GOTO_IF((fcc_status != FCC_STATUS_SUCCESS), fcc_status,exit, "Failed to verify_existence_and_set_warning");

    //Check server uri data
    fcc_status = fcc_check_uri_contents(server_uri_buffer, item_size);
...
static fcc_status_e check_utc_offset(void)
{
    ...
    if (fcc_status == FCC_STATUS_ITEM_NOT_EXIST || fcc_status == FCC_STATUS_EMPTY_ITEM) {
        fcc_output_status = fcc_store_warning_info(parameter_name, size_of_parameter_name, g_fcc_utc_offset_is_not_set_warning_str);
...
*        fcc_status_e status.
*/
static fcc_status_e verify_device_private_key(bool use_bootstrap)
{
    fcc_status_e fcc_status = FCC_STATUS_SUCCESS;
    fcc_status_e output_info_fcc_status = FCC_STATUS_SUCCESS;
    kcm_status_e kcm_status = KCM_STATUS_SUCCESS;
    ...
    //Check that device private key of current mode is present
    kcm_status = kcm_item_get_data_size((const uint8_t*)parameter_name,
                                        size_of_parameter_name,
                                        KCM_PRIVATE_KEY_ITEM,
                                        &item_size);
    SA_PV_ERR_RECOVERABLE_GOTO_IF((kcm_status != KCM_STATUS_SUCCESS), fcc_status = FCC_STATUS_ITEM_NOT_EXIST, store_error_and_exit, "Failed to get device private key size");
    SA_PV_ERR_RECOVERABLE_GOTO_IF((item_size == 0), fcc_status = FCC_STATUS_EMPTY_ITEM, store_error_and_exit, "Device private key is empty");
...
*        fcc_status_e status.
*/
static fcc_status_e verify_certificate_expiration(const uint8_t *certificate_data, size_t size_of_certificate_data, const uint8_t *certificate_name, size_t size_of_certificate_name)
{
    fcc_status_e fcc_status = FCC_STATUS_SUCCESS;
    fcc_status_e output_info_fcc_status = FCC_STATUS_SUCCESS;
    ...
    SA_PV_LOG_TRACE_FUNC_ENTER_NO_ARGS();
    //Check "valid from" attribute

    //Get "valid_from" certificate attribute
    fcc_status = fcc_get_certificate_attribute(certificate_data, size_of_certificate_data, CS_VALID_FROM_ATTRIBUTE_TYPE, (uint8_t**)&valid_from_attr, &size_of_valid_from_attr);
    ...
    fcc_status = fcc_get_certificate_attribute(certificate_data, size_of_certificate_data, CS_VALID_TO_ATTRIBUTE_TYPE, (uint8_t**)&valid_until_attr, &size_of_valid_until_attr);
...
exit:
    free(valid_from_attr);
    free(valid_until_attr);
    if (fcc_status != FCC_STATUS_SUCCESS) {
...
*        fcc_status_e status.
*/
static fcc_status_e compare_ou_with_aid_server(const uint8_t *certificate_data, size_t size_of_certificate_data)
{
    fcc_status_e fcc_status = FCC_STATUS_SUCCESS;
    ...
    fcc_status = fcc_get_certificate_attribute(certificate_data, size_of_certificate_data, CS_OU_ATTRIBUTE_TYPE, &ou_attribute_data, &ou_attribute_size);
...
*        fcc_status_e status.
*/
static fcc_status_e compare_cn_with_endpoint(const uint8_t *certificate_data, size_t size_of_certificate_data)
{
    kcm_status_e kcm_status = KCM_STATUS_SUCCESS;
    fcc_status_e fcc_status = FCC_STATUS_SUCCESS;
    //fcc_status_e output_info_fcc_status = FCC_STATUS_SUCCESS;
    ...
    fcc_status = fcc_get_certificate_attribute(certificate_data, size_of_certificate_data, CS_CN_ATTRIBUTE_TYPE, &cn_attribute_data, &size_of_cn_attr);
    SA_PV_ERR_RECOVERABLE_RETURN_IF(fcc_status != FCC_STATUS_SUCCESS, fcc_status = fcc_status, "Failed to get size OU attribute");

    //Get endpoint name size
    fcc_status = fcc_get_kcm_data(g_fcc_endpoint_parameter_name, strlen(g_fcc_endpoint_parameter_name), KCM_CONFIG_ITEM, &endpoint_name_data, &endpoint_name_size);
    SA_PV_ERR_RECOVERABLE_GOTO_IF((fcc_status != FCC_STATUS_SUCCESS), fcc_status = fcc_status, exit, "Failed to get endpoint name");

    result = memcmp(endpoint_name_data, cn_attribute_data, endpoint_name_size);
...
exit:
    free(cn_attribute_data);
    free(endpoint_name_data);
    return fcc_status;
}
/**This function verifies device certificate
*
* @param device_objects[in]                  pointer to device object structure.
* @param use_bootstrap[in]                   bootstrap mode
*        fcc_status_e status.
*/
static fcc_status_e verify_device_certificate(bool use_bootstrap)
{
    fcc_status_e fcc_status = FCC_STATUS_SUCCESS;
    fcc_status_e output_info_fcc_status = FCC_STATUS_SUCCESS;
    kcm_status_e kcm_status = KCM_STATUS_SUCCESS;
    ...
    fcc_status = fcc_get_kcm_data(parameter_name, size_of_parameter_name, KCM_CERTIFICATE_ITEM, &device_cert, &item_size);
    SA_PV_ERR_RECOVERABLE_GOTO_IF((fcc_status != FCC_STATUS_SUCCESS), fcc_status = fcc_status, exit,"Failed to get device certificate");
    ...
    fcc_status = compare_cn_with_endpoint(device_cert, item_size);
    SA_PV_ERR_RECOVERABLE_GOTO_IF((fcc_status != FCC_STATUS_SUCCESS), fcc_status = fcc_status,exit, "Failed to compare_cn_with_endpoint");
    ...
    if (strcmp((const char*)parameter_name, g_fcc_lwm2m_device_certificate_name) == 0) {
        fcc_status = compare_ou_with_aid_server(device_cert, item_size);
        SA_PV_ERR_RECOVERABLE_GOTO_IF((fcc_status != FCC_STATUS_SUCCESS), fcc_status = fcc_status, exit, "Failed to compare_ou_with_aid_server");
    }

    //Check that device certificate not self-signed
    kcm_status = cs_is_self_signed_der_x509_cert(device_cert, item_size, &is_self_signed);
    SA_PV_ERR_RECOVERABLE_GOTO_IF((kcm_status != KCM_STATUS_SUCCESS), fcc_status = FCC_STATUS_INVALID_CERTIFICATE, exit, "Failed to check if device certificate is self-signed");
    ...
    fcc_status = verify_certificate_expiration(device_cert, item_size, parameter_name, size_of_parameter_name);
...
*/
static fcc_status_e verify_firmware_integrity_ca_certificate(void)
{
    kcm_status_e kcm_status = KCM_STATUS_SUCCESS;
    fcc_status_e fcc_status = FCC_STATUS_SUCCESS;
    fcc_status_e fcc_output_status = FCC_STATUS_SUCCESS;
    fcc_status_e output_info_fcc_status = FCC_STATUS_SUCCESS;
    uint8_t *parameter_name = (uint8_t*)g_fcc_firmware_integrity_ca_certificate_name;
    size_t size_of_parameter_name = strlen(g_fcc_firmware_integrity_ca_certificate_name);
    size_t ca_certificate_data_size = 0;
    uint8_t *ca_certificate_data = NULL;
    ...
    fcc_status = fcc_get_kcm_data(parameter_name, size_of_parameter_name, KCM_CERTIFICATE_ITEM, &ca_certificate_data, &ca_certificate_data_size);

    if (fcc_status == FCC_STATUS_ITEM_NOT_EXIST || fcc_status == FCC_STATUS_EMPTY_ITEM) {
        fcc_output_status = fcc_store_warning_info((const uint8_t*)parameter_name, size_of_parameter_name, g_fcc_missing_parameter_warning_str);
        SA_PV_ERR_RECOVERABLE_GOTO_IF((fcc_output_status != FCC_STATUS_SUCCESS),
            fcc_status = FCC_STATUS_WARNING_CREATE_ERROR,
            exit,
            "Failed to create output warning %s",
        ...
        //If get kcm data returned error, exit with error
        SA_PV_ERR_RECOVERABLE_GOTO_IF((fcc_status != FCC_STATUS_SUCCESS), fcc_status = fcc_status, exit, "Failed to get ca certificate data");

        //Check if ca certificate is self-signed
        kcm_status = cs_is_self_signed_der_x509_cert(ca_certificate_data, ca_certificate_data_size, &is_self_signed);
        SA_PV_ERR_RECOVERABLE_GOTO_IF((kcm_status != KCM_STATUS_SUCCESS), fcc_status = FCC_STATUS_INVALID_CERTIFICATE, exit, "Failed to check if device certificate is self-signed");
    }

    parameter_name = (uint8_t*)g_fcc_firmware_integrity_certificate_name;
    size_of_parameter_name = strlen(g_fcc_firmware_integrity_certificate_name);
    ...
    if (fcc_status == FCC_STATUS_ITEM_NOT_EXIST || fcc_status == FCC_STATUS_EMPTY_ITEM) {
        ...
        //Check firmware integrity certificate expiration
        fcc_status = verify_certificate_expiration(certificate_data, certificate_data_size, parameter_name, size_of_parameter_name);
        SA_PV_ERR_RECOVERABLE_GOTO_IF((fcc_status != FCC_STATUS_SUCCESS), fcc_status = fcc_status, exit, "Failed to verify_certificate_validity");
        if (is_self_signed) {
            kcm_status = cs_verify_der_x509_cert(certificate_data, certificate_data_size, true, ca_certificate_data, ca_certificate_data_size);
            SA_PV_ERR_RECOVERABLE_GOTO_IF((kcm_status == KCM_CRYPTO_STATUS_CERT_NOT_TRUSTED), fcc_status = FCC_STATUS_INVALID_CA_CERT_SIGNATURE, exit, "Failed to verify against ca certificate");
            SA_PV_ERR_RECOVERABLE_GOTO_IF((kcm_status != KCM_STATUS_SUCCESS), fcc_status = FCC_STATUS_INVALID_CERTIFICATE, exit, "Invalid certificate");
        }
    }

exit:
    free(ca_certificate_data);
...
fcc_status_e fcc_check_time_synchronization()
{
    ...
    */
    ...
    time = pal_osGetTime();
    if (time == 0) {
        output_info_fcc_status = fcc_store_warning_info((const uint8_t*)g_fcc_current_time_parameter_name, strlen(g_fcc_current_time_parameter_name), g_fcc_time_is_not_set_warning_str);
    ...
    if (kcm_status != KCM_STATUS_SUCCESS || item_size == 0) {
        output_info_fcc_status = fcc_store_warning_info(parameter_name, size_of_parameter_name, g_fcc_time_zone_is_not_set_warning_str);
        SA_PV_ERR_RECOVERABLE_RETURN_IF((output_info_fcc_status != FCC_STATUS_SUCCESS),
                                        fcc_status = FCC_STATUS_WARNING_CREATE_ERROR,
                                        "Failed to create output warning %s",
                                        g_fcc_time_zone_is_not_set_warning_str);
    }
...
fcc_status_e fcc_check_device_security_objects(bool use_bootstrap)
{
    ...
    fcc_status = verify_device_private_key(use_bootstrap);
    SA_PV_ERR_RECOVERABLE_RETURN_IF((fcc_status != FCC_STATUS_SUCCESS), fcc_status, "Failed to verify device private key ");
    ...
    fcc_status = verify_device_certificate(use_bootstrap);
    SA_PV_ERR_RECOVERABLE_RETURN_IF((fcc_status != FCC_STATUS_SUCCESS), fcc_status, "Failed to verify device certificate");
...
fcc_status_e fcc_check_firmware_update_integrity(void)
{
    ...
    fcc_status = verify_firmware_integrity_ca_certificate();
