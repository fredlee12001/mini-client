...
static fcc_status_e check_and_set_time(const fcc_bundle_data_param_s *current_time)
{
    ...
    // Firstly - check if someone set the time beforehand, if yes, no need to set the time again.
    time = pal_osGetTime();
    if (time != 0) {
...
fcc_status_e fcc_bundle_process_config_params(const cn_cbor *config_params_list_cb)
{
    ...
    for (config_param_index = 0; config_param_index < (uint32_t)config_params_list_cb->length; config_param_index++) {
        ...
        // Sets the time if hasn't been set before
        if (memcmp(config_param.name, g_fcc_current_time_parameter_name, currentTimeLength) == 0) {
            fcc_status = check_and_set_time(&config_param);
            SA_PV_ERR_RECOVERABLE_GOTO_IF((fcc_status != FCC_STATUS_SUCCESS), fcc_status = fcc_status, exit, "check_and_set_time failed");
        } else {
