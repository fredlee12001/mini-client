...
static bool prepare_reponse_message(uint8_t **bundle_response_out, size_t *bundle_response_size_out, fcc_status_e fcc_status)
{
    ...
    */
    ...
    //Allocate out buffer
    out_buffer = malloc(size_of_cbor_buffer);
...
static bool check_scheme_version(cn_cbor *cbor_blob)
{
    ...
    result = memcmp(scheme_version_cb->v.bytes, fcc_bundle_scheme_version, scheme_version_cb->length);
    SA_PV_ERR_RECOVERABLE_RETURN_IF((result != 0), false, "Wrong scheme version");
...
fcc_status_e  fcc_bundle_handler(const uint8_t *encoded_blob, size_t encoded_blob_size, uint8_t **bundle_response_out, size_t *bundle_response_size_out)
{
    ...
    *bundle_response_out = NULL;
    ...
    /*Initialize fcc_output_info_s structure , in case of error during store proccess the
        ...
        if (group_value_cb != NULL) {
            ...
            switch (group_type) {
                ...
                case FCC_CONFIG_PARAM_GROUP_TYPE:
                    ...
                    SA_PV_ERR_RECOVERABLE_GOTO_IF((fcc_status != FCC_STATUS_SUCCESS), fcc_status = fcc_status, free_cbor_list_and_out, "process_general_params failed");
                    break;
